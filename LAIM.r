  ...
  with(as.list(c(state,parms)),{
    ...
    if (atmrespondTF) {
      ...
      if (ABLTF) {
        ...
        dhprime.dt<-(1+2*Beta)*F0thetav/(gamma*h)  # Eq. (6.18) of Garratt [1992]
        dh.dt<-(1+2*Beta)*F0thetav/(rho.surf*gamma*h)  # Eq. (6.18) of Garratt [1992]
        print("dh.dt,dhprime.dt,rho.surf:",paste(signif(dh.dt,5),signif(dhprime.dt,5),signif(rho.surf,5)))
